...
par(mfrow = c(1,1),mar = c(2,13,2,1), xpd = NA)
cartoon.plot(phy, type="fan", show.tip.label=FALSE, auto.polies=TRUE, clade.col="grey40")
tipring(tips=which(myc.assoc=="AM" ), col=alpha(cols1[1]), lwd=8, radial.adj=1.1)
tipring(tips=which(myc.assoc=="ECM"), col=alpha(cols1[2]), lwd=8, radial.adj=1.1)
...
#mycorrhizal legend.
mtext(expression(paste(bold('mycorrhizal association'))), line = -3.2, adj = -.775)
